...
router.post('/submit', authMiddleware, upload.single('image'), async (req, res) => {
    try {
        ...
        if (!['Plastic', 'Paper', 'Metal', 'EWaste', 'Organic'].includes(category)) {
        ...
        const submissionRef = await db.collection('wasteSubmissions').add(submissionData);
...
router.get('/submissions', authMiddleware, async (req, res) => {
    try {
        ...
        for (const doc of submissionsSnapshot.docs) {
            ...
            if (data.verifierId) {
                const verifierDoc = await db.collection('users').doc(data.verifierId).get();
...
router.get('/submission/:id', authMiddleware, async (req, res) => {
    try {
        const submissionDoc = await db.collection('wasteSubmissions').doc(req.params.id).get();
        ...
        if (submission.userId !== req.userId &&
            req.user.role !== 'verifier' && req.user.role !== 'admin') {
        ...
        if (submission.verifierId) {
            const verifierDoc = await db.collection('users').doc(submission.verifierId).get();
