  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        console.error(
          "Blockchain error (using fallback reward calculation):",
          blockchainError
        );
        ...
        console.log(
          `Fallback reward calculated: ${rewardAmount} tokens for ${finalWeight}kg of ${submission.category}`
        );
      }
      ...
      // Check if submission qualifies for certificate (40kg+)
      ...
      if (weightInGrams >= MINIMUM_CERTIFICATE_WEIGHT) {
        try {
          certificate = await issueCertificate(
            ...
            {
              walletAddress: submission.userWallet,
              name: userData.name,
            },
            {
              id: req.userId,
          ...
          if (certificate) {
            ...
            console.log(
              `🏆 Certificate ${certificate.certificateNumber} issued for ${finalWeight}kg submission`
            );
          }
        } catch (certError) {
          console.error("Certificate issuance error:", certError);
          // Don't fail the verification if certificate fails
        }
      }
