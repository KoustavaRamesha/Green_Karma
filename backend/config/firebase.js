...
const FIREBASE_API_KEY = process.env.FIREBASE_API_KEY || 'AIzaSyANqzbG3-13xc35__7cYTdI3NaaLmya6J4';
...
if (SERVICE_ACCOUNT_PATH) {
    try {
        ...
        console.warn('⚠️ Failed to load service account, falling back to REST client:', e.message);
    }
}
...
if (!db) {
    class FirestoreCollection {
        ...
        where() { return new QueryBuilder(this); }
        orderBy() { return new QueryBuilder(this); }
        async add(data) {
            const docId = Date.now().toString();
            const payload = { fields: this._toFirestoreFormat(data) };
            await axios.post(`${this.baseUrl}/${this.collectionName}?key=${FIREBASE_API_KEY}&documentId=${docId}`, payload);
            return { id: docId };
        }
    ...
    class QueryBuilder {
        ...
        limit(num) { this._limit = num; return this; }
        orderBy(field, direction) { this._order = { field, direction }; return this; }
        async get() {
            const url = `${this.collection.baseUrl}/${this.collection.collectionName}?key=${FIREBASE_API_KEY}`;
            const res = await axios.get(url);
            let docs = (res.data.documents || []).map(doc => ({ id: doc.name.split('/').pop(), ...doc }));
            ...
            if (this._where.length > 0) {
                docs = docs.filter(doc => {
                    return this._where.every(filter => {
                        ...
                        const fieldValue = field?.stringValue ?? field?.doubleValue ?? field?.integerValue ?? field?.booleanValue ?? null;
            ...
            if (this._order) {
                docs.sort((a, b) => {
                    ...
                    const aVal = av?.stringValue ?? av?.doubleValue ?? av?.integerValue ?? 0;
                    const bVal = bv?.stringValue ?? bv?.doubleValue ?? bv?.integerValue ?? 0;
            ...
            return {
                empty: docs.length === 0,
                docs: docs.map(d => ({ id: d.id, data: () => this.collection._convertFirestoreData(d.fields) }))
            };
        }
    }
